...
export default function CommandsPanel({ token, onExpired }) {
  ...
  const loadRegenStatus = useCallback(async () => {
    try {
      ...
      if (r.ok) { const d = await r.json(); setRegenPending(d.pending === true) }
    } catch {}
  }, [token])
  ...
  async function queueRegen() {
    try {
      const r = await fetch('/api/admin/regen', { method: 'POST', headers: authHeaders(token) })
      if (r.status === 403) { onExpired(); return }
      setRegenPending(true); setRegenOpen(false)
    } catch {}
  }
  async function cancelRegen() {
    try {
      const r = await fetch('/api/admin/regen', { method: 'DELETE', headers: authHeaders(token) })
      if (r.status === 403) { onExpired(); return }
  ...
  async function submitInput(e) {
    ...
    await exec(input.trim()); setInput(''); setHistIdx(-1)
  }
  ...
  function handleKeyDown(e) {
    if (e.key === 'ArrowUp') {
      ...
      setHistIdx(next); if (history[next]) setInput(history[next].cmd)
    } else if (e.key === 'ArrowDown') {
      e.preventDefault()
      const next = Math.max(histIdx - 1, -1)
      setHistIdx(next); setInput(next === -1 ? '' : history[next]?.cmd ?? '')
    }
  }

  async function sendBroadcast() {
    if (!broadcast.trim()) return
    await exec(`say ${broadcast.trim()}`); setBc(''); setBcOpen(false)
  }

  return (
    <div className={styles.wrap}>
      <div className={styles.group}>
        <h3 className={styles.groupLabel}>Server</h3>
        <div className={styles.grid}>
          ...
          <button className={`${styles.quickBtn} ${styles.quickBroadcast}`} onClick={() => setBcOpen(true)}><span className={styles.quickIcon}>📢</span>Broadcast</button>
          <button className={`${styles.quickBtn} ${styles.quickWarn}`} onClick={() => setEndResetOpen(true)}><span className={styles.quickIcon}>🌍</span>Queue End Reset</button>
          <button className={`${styles.quickBtn} ${styles.quickWarn}`} onClick={() => regenPending ? cancelRegen() : setRegenOpen(true)}><span className={styles.quickIcon}>🌿</span>{regenPending ? 'Cancel Regen' : 'Regen Wilderness'}</button>
          <button className={`${styles.quickBtn} ${styles.quickStop}`} onClick={() => setStopOpen(true)}><span className={styles.quickIcon}>⏹</span>Stop Server</button>
        </div>
      </div>

...
import { Toolbar, IconButton, Btn, Badge, Loading, EmptyState, ErrorBanner, ConfirmDialog, Field, Input, Select, Toggle, useToast } from '../../ui'
...
export default function CommandBlocksPanel({ token, onExpired }) {
  ...
  function copyTp(b) {
    navigator.clipboard.writeText(`/tp @s ${b.x} ${b.y} ${b.z}`).then(() => {
      setCopied(blockKey(b)); setTimeout(() => setCopied(null), 1500)
    })
  }
              ...
              <div key={key}>
                <div className={styles.row}>
                  <div className={styles.info}>
                    <span className={styles.loc}>{dimName(b.dim)} [{b.x}, {b.y}, {b.z}]</span>
                  ...
                  <div className={styles.flags}>
                    ...
                    {b.conditional && <Badge variant="warn">Conditional</Badge>}
                    {b.auto && <Badge variant="ok">Always Active</Badge>}
                  </div>
                  <div className={styles.actions}>
                    <Btn size="sm" onClick={() => copyTp(b)}>{copied === key ? 'Copied!' : 'TP'}</Btn>
                    <Btn size="sm" onClick={() => isEditing ? setEditing(null) : startEdit(b)}>{isEditing ? 'Cancel' : 'Edit'}</Btn>
                    <Btn size="sm" variant="danger" onClick={() => setConfirmDel(b)}>Delete</Btn>
                  </div>
                </div>
                ...
                {isEditing && (
                  <div className={styles.editForm}>
                    <Field label="Command">
                      <Input value={editState.command} onChange={e => setEditState(s => ({ ...s, command: e.target.value }))} style={{ fontFamily: 'var(--font-mono)' }} />
                    </Field>
                    <div className={styles.editControls}>
                      <Field label="Mode">
                        <Select value={editState.mode} onChange={e => setEditState(s => ({ ...s, mode: e.target.value }))}>

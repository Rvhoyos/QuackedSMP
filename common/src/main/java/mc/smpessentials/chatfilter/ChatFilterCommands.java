        ...
        dispatcher.register(Commands.literal("chatfilter")
            ...
            .then(Commands.literal("add")
                .then(Commands.argument("word", StringArgumentType.word())
                    .executes(ctx -> {
                        ...
                        if (ok) {
                            src.sendSuccess(() -> Component.literal("Added to chat filter: " + ChatFilter.normalize(word)), false);
                        } else {
                            src.sendFailure(Component.literal("Already present: " + ChatFilter.normalize(word)));
                        }
                        return ok ? 1 : 0;
                    })
                )
            ...
            .then(Commands.literal("remove")
                .then(Commands.argument("word", StringArgumentType.word())
                    .executes(ctx -> {
                        ...
                        if (ok) {
                            src.sendSuccess(() -> Component.literal("Removed from chat filter: " + ChatFilter.normalize(word)), false);
                        } else {
                            src.sendFailure(Component.literal("Not found: " + ChatFilter.normalize(word)));
                        }
                        return ok ? 1 : 0;
                    })
                )
            ...
            .then(Commands.literal("list")
                .executes(ctx -> {
                    ...
                    String joined = words.isEmpty() ? "(empty)" : String.join(", ", words);
                    src.sendSuccess(() -> Component.literal("Chat filter words: " + joined), false);
                    return words.size();
                })
            )

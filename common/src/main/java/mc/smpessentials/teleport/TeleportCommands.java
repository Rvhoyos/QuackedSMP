...
public final class TeleportCommands {
                            ...
                            switch (r) {
                                case OK -> {
                                    requester.displayClientMessage(
                                            Component.literal(
                                                    "Teleport request sent to " + target.getGameProfile().getName()),
                                            false);
                                ...
                                case SELF_TARGET ->
                                    src.sendFailure(Component.literal(SmpConfig.MESSAGES.getOrDefault("tpr.self",
                                            "Cannot request teleport to self.")));
                                case IN_COOLDOWN ->
                                    src.sendFailure(Component.literal(SmpConfig.MESSAGES.getOrDefault("tpr.cooldown",
                                            "A request was sent recently. Please wait.")));
                                case ALREADY_PENDING ->
                                    src.sendFailure(
                                            Component.literal(SmpConfig.MESSAGES.getOrDefault("tpr.already_pending",
                                                    "A request is already pending.")));
                                case TARGET_QUEUE_FULL ->
                                    src.sendFailure(Component.literal(SmpConfig.MESSAGES.getOrDefault("tpr.queue_full",
    ...
    private static int acceptOldest(CommandSourceStack src) throws CommandSyntaxException {
        ...
        if (requester.isPassenger() || requester.isSleeping()) {
            src.sendFailure(Component.literal(SmpConfig.MESSAGES
                    .getOrDefault("tpa.requester_busy", "Requester cannot be teleported right now.")));
            return 0;
        }
        mc.smpessentials.teleport.TeleportScheduler.schedule(requester, () -> {
            ...
            if (!ok) {
                requester.displayClientMessage(Component.literal(SmpConfig.MESSAGES
                        .getOrDefault("tpa.no_location", "No valid teleport location.")), false);
                ...
                requester.displayClientMessage(Component.literal(
                        toMsg.replace("{player}", target.getGameProfile().getName())), false);
                target.displayClientMessage(
                        Component.literal(requester.getGameProfile().getName() + " has been teleported."), false);
            }
        });
        ...
        target.displayClientMessage(
                Component.literal(telMsg.replace("{player}", requester.getGameProfile().getName())),
                false);
        return 1;
    }
    ...
    private static int denyOldest(CommandSourceStack src) throws CommandSyntaxException {
        ...
        if (requester != null) {
            ...
            requester.displayClientMessage(
                    Component.literal(deniedMsg.replace("{player}", target.getGameProfile().getName())),
                    false);
        }
